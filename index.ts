...
(async () => {
  ...
  //const uniswapABI = IUniswapV2Router02.abi;

  const provider = ethers.getDefaultProvider('homestead');
  //const uniswapContract = new ethers.Contract(uniswapAddress, uniswapABI);

  const coingeckoApi = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${FROM_TOKEN.coingecko}%2c${TO_TOKEN.coingecko}&vs_currencies=usd&precision=18`);

  const listOfSpotValues = await coingeckoApi.json();
  ...
  //console.info(uniswapContract);

  //uniswapContract.connect(provider);
  // Use ethers and the DEX contract to figure out how much TO_TOKEN you can get
  // for the FROM_TOKEN.

  //let numNewTokens: number = uniswapContract.methods.swapExactTokensForTokens(FROM_TOKEN, TO_TOKEN, FROM_BALANCE);

  //const fromSpotPrice: number = uniswapContract.methods.spotPrice(FROM_TOKEN);
  //const toSpotPrice: number = uniswapContract.methods.spotPrice(TO_TOKEN);

  //console.info(`From Spot Price: ${fromSpotPrice.toString()}`);
  //console.info(`To Spot Price: ${toSpotPrice.toString()}`);
